...
// Single-buyer "offer war-room". Once the field is down to one bidder the deal
// is no longer a competition to handicap — it's one offer to judge. This card
// grades the received offer against the expected market band, against a
// walk-away floor the operator sets, and reads out a plain verdict. Rendered
// only when exactly one buyer is live (see App). The offer is persisted in the
// localStorage workspace state under `offer` — it's a real deal artifact, not a
// transient what-if.
export function OfferWarRoom({ buyers, ebitda, caseMode, market, offer, onSetOffer }) {
  const live = (buyers || []).filter(b => b.stage !== 'dropped');
  const buyer = live[0];
  const seed = marketMultiplesSeed(ebitda);
  const band = (market && market[caseMode]) || seed[caseMode] || seed.mid;
  const cons = (market && market.conservative) || seed.conservative;
  const aggr = (market && market.aggressive) || seed.aggressive;

  const bandLow = ebitda * band.low;
  const bandHigh = ebitda * band.high;
  const bandMid = ebitda * ((band.low + band.high) / 2);

  // Scale spans bear-low → bull-high so the offer reads against the full case
  // spread, with light padding so the end markers aren't clipped.
  const rawLow = ebitda * cons.low;
  const rawHigh = ebitda * aggr.high;
  const pad = (rawHigh - rawLow) * 0.08 || 1;
  const scaleLow = rawLow - pad;
  const scaleHigh = rawHigh + pad;
  const pct = (x) => Math.max(0, Math.min(100, ((x - scaleLow) / (scaleHigh - scaleLow)) * 100));

  const amount = offer && typeof offer.amount === 'number' ? offer.amount : null;
  const defaultFloor = Math.round(bandLow);
  const floor = offer && typeof offer.floor === 'number' ? offer.floor : defaultFloor;
  const set = (patch) => onSetOffer({ amount: null, floor: null, ...(offer || {}), ...patch });

  const offerMult = amount != null && ebitda ? amount / ebitda : null;
  let verdict = null, tone = 'neutral';
  if (amount != null) {
    const label = (band.label || 'realistic').toLowerCase();
    if (amount >= bandHigh) { verdict = `Above the ${label} band — strong. Drive to signing.`; tone = 'good'; }
    else if (amount >= bandLow) { verdict = `Inside the ${label} band — a fair market clear.`; tone = 'good'; }
    else if (amount >= floor) { verdict = `Below the band but above your floor — push for more before signing.`; tone = 'warn'; }
    else { verdict = `Below your walk-away floor — reject or re-trade.`; tone = 'bad'; }
  }
  const deltaMid = amount != null ? Math.round(amount - bandMid) : null;
  ...
  return (
    <div className="warroom">
      <div className="warroom-head">
        ...
        <b>Offer war-room</b>
        <span className="warroom-sub">{buyer ? buyer.name : 'sole buyer'} · grade the offer against market, set your floor</span>
      </div>

      <div className="warroom-gauge-wrap">
        <div className="warroom-track">
          <div className="warroom-band" style={{ left: pct(bandLow) + '%', width: (pct(bandHigh) - pct(bandLow)) + '%' }} title={`${band.label} band ${fmtMoney(bandLow)}–${fmtMoney(bandHigh)}`} />
          <div className="warroom-tick warroom-tick-mid" style={{ left: pct(bandMid) + '%' }} title={`Expected ${fmtMoney(bandMid)}`} />
          <div className="warroom-tick warroom-tick-floor" style={{ left: pct(floor) + '%' }} title={`Walk-away floor ${fmtMoney(floor)}`} />
          {amount != null && (
            <div className={'warroom-marker warroom-marker-' + tone} style={{ left: pct(amount) + '%' }} title={`Offer ${fmtMoney(amount)}`} />
          )}
        </div>
        <div className="warroom-scale">
          <span>{fmtMoney(scaleLow)}</span>
          <span className="warroom-scale-mid">{band.label} {fmtMoney(bandLow)}–{fmtMoney(bandHigh)} · {band.low.toFixed(1)}–{band.high.toFixed(1)}×</span>
          <span>{fmtMoney(scaleHigh)}</span>
        </div>
      </div>

      <div className="warroom-grid">
        <label className="warroom-field">
          <span>Offer received · total EV $M</span>
          <input type="number" inputMode="decimal" placeholder="awaiting LOI"
            value={amount ?? ''}
            onChange={(e) => set({ amount: e.target.value === '' ? null : Number(e.target.value) })} />
        </label>
        <label className="warroom-field">
          <span>Walk-away floor · $M</span>
          <input type="number" inputMode="decimal" placeholder={String(defaultFloor)}
            value={offer && typeof offer.floor === 'number' ? offer.floor : ''}
            onChange={(e) => set({ floor: e.target.value === '' ? null : Number(e.target.value) })} />
        </label>
        <div className="warroom-readout">
          {amount != null ? (
            <>
              <div className="warroom-readout-num">{fmtMoney(amount)} <span>· {offerMult.toFixed(1)}×</span></div>
              <div className={'warroom-verdict warroom-verdict-' + tone}>{verdict}</div>
              <div className="warroom-readout-sub">{deltaMid >= 0 ? '+' : '−'}{fmtMoney(Math.abs(deltaMid))} vs expected {fmtMoney(bandMid)} · floor {fmtMoney(floor)}</div>
            </>
          ) : (
            <>
              <div className="warroom-readout-num warroom-readout-await">Awaiting LOI</div>
              <div className="warroom-readout-sub">Target {fmtMoney(bandLow)}–{fmtMoney(bandHigh)} · floor {fmtMoney(floor)}. Enter the offer when it lands to grade it.</div>
            </>
          )}
        </div>
      </div>
...
**Do NOT cite specific probability numbers in your replies.** The buyer probability values you see (e.g., \`probability: 27\`) are RAW blended values BEFORE winner-share normalization. The dashboard displays each buyer's **winner-allocated share** (e.g., 17%) — a different, smaller number. If you write "Kelly should firm up from 27" the user sees Kelly at 17% on screen and rightly concludes you're hallucinating. Use directional language instead: "Kelly should firm up", "Oakbridge should compress toward Trucordia", "the gap should widen". Cite a number only when it appears on the dashboard the user can verify (close month, close confidence %, deal-confidence KPI, market clearing band). Stage names, milestones, and dates from the noteLog are fair game; per-buyer probability percentages are not.
