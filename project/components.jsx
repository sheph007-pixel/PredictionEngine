// Kennion Prediction Engine — components

const { useState, useEffect, useRef, useMemo } = React;
const { STAGES, STAGE_INDEX, PROCESS_TASKS, PHASES, PROCESS_DEFAULT, BUYERS } = window.KENNION;
...
// Project all task dates from current anchor
function projectTaskDates(process) {
  ...
  const anchorDate = new Date(process.currentTaskDate);
...
// P(winner | this is the closer): normalized across live buyers using each
// buyer's "interest score" (their independent probability * deal-value attractiveness).
// Reserves a residual for P(no deal). Returns a map { buyerId: pctWinner }
// AND noDealPct, all summing to 100. This is the right framing for a single-asset
// auction where exactly one buyer wins.
function winnerProbabilities(buyers, ebitda = 18, caseMode = "mid") {
  const live = buyers.filter(b => b.stage !== "dropped");
  if (live.length === 0) return { winnerByBuyer: {}, noDealPct: 100, dealClosesPct: 0 };

  // P(any deal closes) = 1 - ∏(1 - p_i)
  ...
  // Normalize each buyer's "winner score" — heavier weight to advanced-stage buyers
  // and stronger fit. We use the independent probability as the core signal but
  // also factor stage progression so chemistry/LOI buyers are favored.
  ...
  // Distribute the "deal closes" probability mass across buyers in proportion to score
  const winnerByBuyer = {};
  let assigned = 0;
  scores.forEach((x, i) => {
...
// Week-over-week delta (mocked from buyer.lastWeekWinnerPct if present)
function winnerDelta(buyer, currentPct) {
  if (buyer.lastWeekWinnerPct == null) return 0;
  return currentPct - buyer.lastWeekWinnerPct;
}

// Valuation: takes adjusted EBITDA from Reagan + a case (conservative|mid|aggressive)
// and computes per-buyer ranges. Multiples tighten as stage advances.
function valuationFor(buyer, ebitda = 18, caseMode = "mid") {
  ...
  // pick the case multiple for headline numbers
...
// ---------- hero KPIs (the three top-level numbers) ----------
function HeroKPIs({ buyers, process, ebitda, caseMode, market }) {
  ...
  // Market clearing price = EBITDA × the active case multiple range.
  // Pure market answer, independent of buyer mix. Per-buyer predictions
  // appear in the pipeline rows below.
...
// ---------- most likely closer ----------
// Ranks buyers by P(winner) — the AI's prediction of which buyer we close with.
// All live buyers' P(winner) sum to P(deal closes). The "no deal" bar shows
// the residual. The dollar shown alongside each buyer is THEIR likely deal value
// (not a contribution / weighted figure).
function ContributionChart({ buyers, ebitda, caseMode, onSelect }) {
  const live = buyers.filter(b => b.stage !== "dropped");
...
// ---------- process tracker (top) ----------
// Derives projected close from today + current step; no per-task dates shown.
function ProcessTracker({ process, onUpdate, buyers = [], ebitda = 18, caseMode = "mid" }) {
...
// ---------- valuation bar (Reagan EBITDA + case toggle) ----------
// Market multiple ranges — AI-inferred from recent industry M&A signals
// (announced deals, sector trades, broker chatter, public-comp drift).
// Conservative = bear case, Realistic = base case, Aggressive = bull case.
// These are the SEED values; once "Re-scan market" runs, the live values
// flow through props (App owns market state).
...
const MARKET_MULTIPLES = MARKET_MULTIPLES_SEED; // backwards-compat for any consumers

// ---------- system bar (top-right cluster: EBITDA + case + re-scan) ----------
function SystemBar({ ebitda, onEbitda, caseMode, onCase, market, marketMeta, onRescan }) {
...
function ValuationBar({ ebitda, onEbitda, caseMode, onCase, market, marketMeta, onRefreshMarket }) {
  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState(String(ebitda));
  const [refreshing, setRefreshing] = useState(false);
  useEffect(() => setDraft(String(ebitda)), [ebitda]);
...
// ---------- pipeline summary ----------
function PipelineStats({ buyers, ebitda, caseMode, process }) {
...
// ---------- add buyer form ----------
function AddBuyerForm({ onAdd, onCancel, existingBuyers }) {
    ...
    const sys = `You are an M&A analyst building a buyer profile for the Kennion Benefits Program sale (advised by Reagan Consulting). The user is adding a new prospective acquirer to the pipeline. Return ONLY a JSON object — no markdown, no commentary — with this exact shape:
{
  ...
  "multiple": [low, mid, high]   // EBITDA multiples as numbers, e.g. [11.5, 13.0, 14.5]
}
...
// ---------- buyer row (cleaner, no profile clutter) ----------
function BuyerRow({ buyer, selected, onSelect, onAdvance, onDrop, displayRank, ebitda, caseMode, winnerPct, winnerDeltaPct, market }) {
  const stageIdx = STAGE_INDEX[buyer.stage];
  ...
  // vs-market delta: buyer's predicted bid (their mid multiple) vs market case midpoint
  const marketCase = (market && market[caseMode]) || { low: 11, high: 13 };
  const marketMid = (marketCase.low + marketCase.high) / 2;
  const vsMarket = v.multMid - marketMid;
  const vsMarketPct = (vsMarket / marketMid) * 100;
...
// ---------- buyer modal (above-the-fold, 3-column) ----------
function BuyerModal({ buyer, onClose, onAdvance, onDrop, onDelete, onUpdateNotes, onAdjustMultiple, ebitda, caseMode }) {
...
// ---------- AI Engine (chat + upload + input) ----------
function AIChat({ buyers, open, onToggle, alwaysOpen }) {
...
Object.assign(window, { ProcessTracker, ValuationBar, SystemBar, PipelineStats, BuyerRow, BuyerModal, AddBuyerForm, AIChat, HeroKPIs, ContributionChart, probabilityFor, winnerProbabilities, valuationFor, fmtMoney });
